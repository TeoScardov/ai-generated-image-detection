...
def plot_training_stats(train_err, val_err, train_loss):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(epochs, train_err*100, label='Training Error')
    plt.plot(epochs, val_err*100, label='Validation Error')
    plt.xlabel('Epochs')
    plt.ylabel('Error (%)')
    plt.legend()
...
def plot_training_stats_multioutput(train_errors, val_errors, train_loss):
    ...
    plt.subplot(2, 2, 1)
    plt.plot(epochs, train_errors['binary']*100, label='Training Error')
    plt.plot(epochs, val_errors['binary']*100, label='Validation Error')
    plt.title('Binary')
    plt.xlabel('Epochs')
    plt.ylabel('Error (%)')
    plt.legend()
    plt.grid(True, which='both', linestyle='--', linewidth=0.5)
    plt.ylim(bottom=0, top=max([err for c in list(train_errors.values()) + list(val_errors.values()) for err in c]))
    plt.xticks(np.arange(min(epochs), max(epochs)+1, max(1, len(epochs)//10)))

    # Plot training and validation multiclass error
    plt.subplot(2, 2, 2)
    plt.plot(epochs, train_errors['multiclass']*100, label='Training Error')
    plt.plot(epochs, val_errors['multiclass']*100, label='Validation Error')
    ...
    plt.subplot(2, 2, 3)
    plt.plot(epochs, train_errors['combined']*100, label='Training Error')
    plt.plot(epochs, val_errors['combined']*100, label='Validation Error')
    plt.title('Combined')
    plt.xlabel('Epochs')
    plt.ylabel('Error (%)')
    plt.legend()
...
def visualise_samples(model, test_ds, labels_map):
    ...
    for i in range(1, cols * rows + 1):
        ...
        plt.imshow(img.resize((224, 224), resample=Image.Resampling.LANCZOS)) # The image is upsampled for better visualization
    plt.show()
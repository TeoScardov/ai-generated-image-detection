...
def plot_gradcam(model, target_layer, dataset, labels_map, sample_idx):
    global gradients
    global activations
    gradients = None
    activations = None
    ...
    label = torch.tensor([dataset.img_labels[sample_idx]], dtype=torch.long, device='cpu') 
    ...
    plt.title(f'Predicted: {labels_map[predicted_label]};   True: {labels_map[true_label]}')
    plt.imshow(img)
    # Overlayed image
    plt.subplot(1, 3, 2)
    plt.axis('off') 
    plt.imshow(img)
    overlay = to_pil_image(heatmap.detach(), mode='F').resize((32,32))
    cmap = colormaps['jet']
